...
get_region_a_leader() {
  for node in patroni-a1 patroni-a2; do
    STATUS=$(docker exec "$node" curl -s -o /dev/null -w "%{http_code}" --max-time 2 http://localhost:8008/leader 2>/dev/null)
    if [ "$STATUS" = "200" ]; then
      echo "$node"
      return 0
    fi
  done
  echo "patroni-a1"  # fallback
}

log "========== FULL CYCLE TEST RUN $RUN_ID =========="

# --- Pre-check: both regions must match before starting ---
A_LEADER=$(get_region_a_leader)
A_MAX=$(docker exec "$A_LEADER" psql -h 127.0.0.1 -U postgres -t -c "SELECT coalesce(max(id),0) FROM dr_test;" 2>/dev/null | tr -d '[:space:]')
...
(
  while true; do
    docker exec "$A_LEADER" psql -h 127.0.0.1 -U postgres -c \
...
LAST_A_ID=$(docker exec "$A_LEADER" psql -h 127.0.0.1 -U postgres -t -c "SELECT max(id) FROM dr_test;" | tr -d '[:space:]')
T_KILL=$(date +%s.%N)
...
A_LEADER=$(get_region_a_leader)
docker exec "$A_LEADER" psql -h 127.0.0.1 -U postgres -c "DROP SUBSCRIPTION IF EXISTS dr_sub_failback;" > /dev/null 2>&1
...
docker exec "$A_LEADER" psql -h 127.0.0.1 -U postgres -c "TRUNCATE dr_test RESTART IDENTITY;" > /dev/null 2>&1
docker exec "$A_LEADER" psql -h 127.0.0.1 -U postgres -c "
...
for i in $(seq 1 30); do
  A_ID=$(docker exec "$A_LEADER" psql -h 127.0.0.1 -U postgres -t -c "SELECT coalesce(max(id),0) FROM dr_test;" 2>/dev/null | tr -d '[:space:]')
...
if [ "$(docker exec "$A_LEADER" psql -h 127.0.0.1 -U postgres -t -c "SELECT coalesce(max(id),0) FROM dr_test;" 2>/dev/null | tr -d "[:space:]")" != "$FINAL_B_ID" ]; then
  ...
  docker exec -i "$A_LEADER" psql -h 127.0.0.1 -U postgres postgres < /tmp/dr_test_data.sql > /dev/null 2>&1
  log "Fallback data copy applied."
fi

# --- Restore forward direction (A -> B) for next run ---
docker exec "$A_LEADER" psql -h 127.0.0.1 -U postgres -c "ALTER SUBSCRIPTION dr_sub_failback DISABLE;" > /dev/null 2>&1
docker exec "$A_LEADER" psql -h 127.0.0.1 -U postgres -c "ALTER SUBSCRIPTION dr_sub_failback SET (slot_name = NONE);" > /dev/null 2>&1
docker exec "$A_LEADER" psql -h 127.0.0.1 -U postgres -c "DROP SUBSCRIPTION dr_sub_failback;" > /dev/null 2>&1
docker exec patroni-b1 psql -h 127.0.0.1 -U postgres -c "DROP PUBLICATION IF EXISTS dr_pub_failback;" > /dev/null 2>&1
...
docker exec "$A_LEADER" psql -h 127.0.0.1 -U postgres -c "DROP PUBLICATION IF EXISTS dr_pub;" > /dev/null 2>&1
docker exec "$A_LEADER" psql -h 127.0.0.1 -U postgres -c "CREATE PUBLICATION dr_pub FOR TABLE dr_test;" > /dev/null 2>&1
docker exec patroni-b1 psql -h 127.0.0.1 -U postgres -c "
CREATE SUBSCRIPTION dr_sub
CONNECTION 'host=$A_LEADER port=5432 dbname=postgres user=replicator password=replpass'
PUBLICATION dr_pub;
" > /dev/null 2>&1

sleep 5
A_FINAL=$(docker exec "$A_LEADER" psql -h 127.0.0.1 -U postgres -t -c "SELECT max(id) FROM dr_test;" | tr -d '[:space:]')
